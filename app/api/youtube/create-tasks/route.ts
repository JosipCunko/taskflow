...
async function createWatchVideoTasks(
  userId: string,
  videos: YouTubeVideo[]
): Promise<string[]> {
  ...
  for (const video of topVideos) {
    try {
      const taskData: TaskToCreateData = {
        ...
        isPriority: false,
        isReminder: true,
        dueDate: endOfCurrentWeek.getTime(),
        tags: [
          "youtube",
          "video",
          video.channelTitle.toLowerCase().replace(/\s+/g, "-"),
        ],
        duration: {
          hours: 0,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (summaryData?.userId !== userId) {
      return NextResponse.json(
        { error: "Unauthorized access to summary" },
        { status: 403 }
      );
    }

    const videos: YouTubeVideo[] = summaryData.videos.map(
      (video: YouTubeVideo & { publishedAt: { toDate: () => Date } }) => ({
        ...video,
