...
interface DynamicWebAppLayoutProps {
  session: Session | null; // Allow session to be null initially
  tasks: Task[];
  children: ReactNode;
}

export default function DynamicWebAppLayout({
  session,
  tasks,
  children,
}: DynamicWebAppLayoutProps) {
  ...
  return (
    <div className="flex h-screen tracking-tight bg-background-625 relative">
      <Button
        variant="secondary"
        className="md:hidden fixed top-3 left-3 z-[60] p-2 backdrop-blur-sm"
