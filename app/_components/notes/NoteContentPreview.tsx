...
export default function NoteContentPreview({
  ...
  const renderContent = (text: string) => {
    ...
    return lines.map((line, index) => {
      ...
      if (line.startsWith("## ")) {
        return (
          <h4 key={index} className="text-base font-semibold text-text-low mt-2 mb-1">
            {line.substring(3)}
          </h4>
        );
      }
      ...
      if (mathMatch) {
        ...
        mathMatch.forEach((match) => {
          ...
          formattedLine = formattedLine.replace(
            match,
            `<span class="text-purple-400 font-semibold bg-purple-500/10 px-1 rounded">${expr}</span>`
          );
        });
        return (
          <p
