...
10. Offline
  Offline is handled in the client and the service worker, not by rewriting `/webapp` in middleware. See `md/offlineSupport.md`.
...
export default withAuth(
  function middleware(req) {
    ...
    const isWebApp = req.nextUrl.pathname.startsWith("/webapp");

    // Redirect authenticated users away from login
    if (isAuthPage && isAuth) {
      return NextResponse.redirect(new URL("/webapp/today", req.url));
    }
